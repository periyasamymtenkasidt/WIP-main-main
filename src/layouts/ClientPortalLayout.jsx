  ...
  useEffect(() => {
    if (client) {
      ...
      if (foundSite) {
        ...
        resolveSiteFileUrls(foundSite).then((resolved) => {
          setSite(resolved);
          setNotifications(resolved.clientNotifications || []);
        });
      }
    }
  }, [client]);

  const updateSite = async (updatedSiteOrFn) => {
    let nextSite;
    if (typeof updatedSiteOrFn === "function") {
      nextSite = updatedSiteOrFn(site);
    } else {
      nextSite = updatedSiteOrFn;
    }
    if (nextSite) {
      saveSite(nextSite);
      const resolved = await resolveSiteFileUrls(nextSite);
      setSite(resolved);
      if (resolved.clientNotifications) {
        setNotifications(resolved.clientNotifications);
      }
    }
  };
