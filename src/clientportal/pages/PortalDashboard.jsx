...
const PortalDashboard = () => {
  ...
  const getDesignStageLabel = (s) => {
    ...
    if (s.designStatus === "Completed") {
      const hasPendingRevisions =
        ...
        !s.revisions.every((r) => r.status === "Completed");
      if (hasPendingRevisions) return "Redesign";

      const MANDATORY_DRAWINGS = [
        "Floor Plan",
        "Furniture Layout",
        "Electrical Layout",
        "Ceiling Layout",
      ...
      const allMandatoryDrawingsApproved =
        s.drawings &&
        MANDATORY_DRAWINGS.every((name) =>
          s.drawings.some(
            (d) =>
              d.name.toLowerCase().includes(name.toLowerCase()) &&
              d.status === "Approved",
  ...
  const totalDrawings =
    site?.drawings?.filter((d) => d.visibleToClient !== false).length || 0;
  ...
  return (
    <div className="p-6 sm:p-8 space-y-6 flex-1 flex flex-col justify-between text-left">
      <div>
        <h4 className="text-sm font-bold text-darkgray uppercase tracking-wider mb-4">
          Project Feed & Status
        </h4>
        <div className="space-y-6">
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-indigo-50/50 to-purple-50/20 border border-indigo-100/40 shadow-sm">
            ...
            <div className="flex justify-between items-center pt-3 border-t border-slate-100 flex-wrap gap-2">
              <div className="flex items-center gap-1.5 text-xs text-slate-600">
                ...
                <span
                  className={`font-bold ${pendingAction !== "None" ? "text-amber-600" : "text-emerald-600"}`}
                >
                  {pendingAction}
                </span>
              </div>
          ...
          <div className="p-5 rounded-2xl bg-white border border-slate-100 shadow-xs space-y-4 text-left">
            ...
            <p className="text-[10.5px] text-text-subtle leading-relaxed bg-slate-50/50 p-3 rounded-xl border border-slate-100/50">
              <strong>Revision Rule:</strong> Complimentary revisions include
              initial design submission and up to 3 revisions. Subsequent change
              requests are billed as paid revisions.
            </p>
          </div>
            ...
            <div className="border-l-2 border-slate-200 pl-4 space-y-4 text-xs text-left">
              <div className="relative">
                ...
                <p className="text-gray-500 mt-0.5">
                  New drawings uploaded for Client review. Visible in Designs &
                  Renders.
                </p>
              </div>
              <div className="relative">
                ...
                <p className="font-bold text-darkgray">
                  Milestone Paid: Token Advance
                </p>
                <p className="text-gray-500 mt-0.5">
                  Booking token payment confirmed and processed.
                </p>
              </div>
            </div>
          </div>
